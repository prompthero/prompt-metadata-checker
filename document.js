(async() => {
    ...
    try {
        ...
        if (element.tagName === 'input') {
            element.addEventListener('change', parseMetadata)
        } else {
            element.addEventListener('drop', parseMetadata)
            document.querySelector(selector + ' > input')?.addEventListener('change', parseMetadata)
        }
    }
    catch (e) {
        console.error('Postie Error:', e)
    }
})()
